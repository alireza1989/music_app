...
var application = function() {
    ...
    var remove_song_from_playlist = function() {
        if (event.target.className.split(" ")[2] == 'song-remove') {

            // Get the playlist ID and Song ID
            window.song_selected = event.target.parentElement;
            //window.play_list_selected = event.target;
            var playlist_selected_name = document.getElementsByClassName('playlist-page-title')[0].innerHTML;
            var song_element = song_selected.getElementsByTagName('span')[0];
            var song_name = song_element.getElementsByClassName('music-title')[0].innerHTML;
            var song_id = '';
            var playlist_id;
            ...
            // Remove the song from Memory and DB and re-draw the list
            var removeSongObj = JSON.stringify({
                'song': song_id
            });
                ...
                success: function(result) {
                    ...
                    for (j in playlists_data.playlists) {
                        if (playlist_selected_name === playlists_data.playlists[j].name) {
                            ...
                            playlists_data.playlists[j].songs.splice(playlists_data.playlists[j].songs.indexOf(song_id), 1);
                            $(song_selected).remove();
    ...
    document.addEventListener('click', remove_song_from_playlist, false);
    ...
    var addPlayList = function() {
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                playlists_data.playlists.push(newPlaylistLocalObj);
